...
var (
	oidPBEWithSHAAnd3KeyTripleDESCBC = asn1.ObjectIdentifier([]int{1, 2, 840, 113549, 1, 12, 1, 3}) // rfc7292#appendix-C PBE-SHA1-3DES
	oidPBEWithSHAAnd128BitRC2CBC     = asn1.ObjectIdentifier([]int{1, 2, 840, 113549, 1, 12, 1, 5}) // rfc7292#appendix-C PBE-SHA1-RC2-128
	oidPBEWithSHAAnd40BitRC2CBC      = asn1.ObjectIdentifier([]int{1, 2, 840, 113549, 1, 12, 1, 6}) // rfc7292#appendix-C PBE-SHA1-RC2-40
...
func (shaWithTripleDESCBC) deriveKey(salt, password []byte, iterations int) []byte {
	return pbkdfEncKey(sha1Hash, salt, password, iterations, 24)
}

func (shaWithTripleDESCBC) deriveIV(salt, password []byte, iterations int) []byte {
	return pbkdfIV(sha1Hash, salt, password, iterations, 8)
}
...
func (shaWith128BitRC2CBC) deriveKey(salt, password []byte, iterations int) []byte {
	return pbkdfEncKey(sha1Hash, salt, password, iterations, 16)
}
...
func (shaWith40BitRC2CBC) deriveKey(salt, password []byte, iterations int) []byte {
	return pbkdfEncKey(sha1Hash, salt, password, iterations, 5)
}
...
func pbes2CipherFor(algorithm pkix.AlgorithmIdentifier, password []byte) (cipher.Block, []byte, error) {
	...
	case params.EncryptionScheme.Algorithm.Equal(oidAES192CBC):
		keyLen = 24
	case params.EncryptionScheme.Algorithm.Equal(oidAES128CBC) || params.EncryptionScheme.Algorithm.Equal(oidSM4CBC):
		keyLen = 16
	default:
	...
	switch {
	case params.EncryptionScheme.Algorithm.Equal(oidAES256CBC) || params.EncryptionScheme.Algorithm.Equal(oidAES192CBC) || params.EncryptionScheme.Algorithm.Equal(oidAES128CBC):
